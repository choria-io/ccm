...
func publishRegistration(ctx context.Context, mgr model.Manager, prop model.ResourceProperties, event *model.TransactionEvent, log model.Logger) error {
	regs := prop.CommonProperties().RegisterWhenStable
	if len(regs) == 0 {
		log.Debug("Skipping registration for resource without registration configuration", "resource", prop.CommonProperties().Name)
		return nil
	}
	if mgr.NoopMode() {
		log.Debug("Skipping registration in noop mode", "resource", prop.CommonProperties().Name, "noop", true)
		return nil
	}
	if event.Failed {
		log.Debug("Skipping registration due to failed event", "resource", prop.CommonProperties().Name, "failed", true)
		return nil
	}
	if !allHealthChecksPassed(event) {
		log.Debug("Skipping registration due to failed health checks", "resource", prop.CommonProperties().Name, "failed", true)
		return nil
	}

	for _, reg := range regs {
		log.Debug("Publishing registration", "resource", prop.CommonProperties().Name, "service", reg.Service)
