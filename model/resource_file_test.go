...
`
	...
	Describe("FileState", func() {
		It("Should have correct structure", func() {
			...
			metadata := state.Metadata.(*FileMetadata)
			Expect(metadata.Checksum).To(Equal("abc123"))
			Expect(metadata.Owner).To(Equal("root"))
			Expect(metadata.Group).To(Equal("root"))
			Expect(metadata.Mode).To(Equal("0644"))
			Expect(metadata.Provider).To(Equal("posix"))
			Expect(metadata.Extended).To(HaveKeyWithValue("key", "value"))
		})
	})
