...
The `hiera` package can be embedded in Go applications. Supply a YAML document and a map of facts; the resolver parses the hierarchy, replaces `{{ lookup('facts.key') }}` placeholders, and merges matching sections.
...
`)
	...
	jout, err := json.MarshalIndent(resolved, "", "  ")
	if err != nil {
		panic(err)
	}
